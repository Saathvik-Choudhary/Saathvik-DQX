...
# MAGIC %md
# MAGIC # DQX Library Feature Tests
...
tips_view = "tips_medallion_input"
tips_df = (
...
- check:
    ...
    arguments:
      expression: total_bill >= tip
      msg: total bill must be greater than or equal to tip
      name: total_bill_ge_tip
  criticality: error
- check:
    function: is_in_list
    arguments:
...
bronze_df = tips_df.limit(1000)

# apply your business logic here
bronze_transformed_df = bronze_df.filter("day in ('Sun', 'Sat')")
...
dq_engine.apply_checks_by_metadata_and_save_in_table(
    input_config=InputConfig(f"global_temp.{tips_view}"),
    checks=checks,
