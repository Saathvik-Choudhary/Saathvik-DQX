...
# MAGIC %md
# MAGIC # Generating Data Quality Metrics with DQX (Test)
# MAGIC
# MAGIC This notebook validates how to generate and persist a set of data quality summary metrics when applying DQX rules to your data. DQX can track aggregate-level quality metrics based on your quality rule definitions. Aggregate-level quality metrics can be tracked for both batch and streaming data processing workloads.
...
# 1. Ensure row_id, total_bill, tip are not null or empty (error-level)
- check:
...
# 2. Warn if total_bill is outside [1, 100]
- check:
    function: is_in_range
    arguments:
      column: total_bill  # define check for a single column
      min_limit: 1
      max_limit: 100
  criticality: warn
  name: total_bill_not_in_range  # optional check name, auto-generated if not provided

# 3. Warn if day is not a valid dining day
- check:
    dimension: validity
    function: is_in_list
    for_each_column:
...
# 4. Error if row_id is not unique across the dataset
- check:
    function: is_unique
    arguments:
      columns:
...
# Open-source input data for all sections
...
tips_with_id_df = tips_df.withColumn(
    "row_id",
