...
# MAGIC %md
# MAGIC # Using DQX for PII Detection (Test)
...
# MAGIC # Environment
# MAGIC
# MAGIC DQX (with PII extras) and the spaCy language models are installed at the cluster level.
...
# Create open-source sample data with derived PII-like text
...
pii_df = tips_df.select(
  F.concat_ws(
    " ",
    F.lit("Customer"),
    F.col("sex"),
    F.lit("email:"),
    F.concat(F.lower(F.col("sex")), F.lit("@example.com")),
    F.lit("phone:"),
    F.concat(F.lit("555-01"), F.lpad(F.col("size").cast("string"), 2, "0")),
  ).alias("val")
)
pii_df = pii_df.unionByName(spark.createDataFrame([("No PII here",), (None,)], ["val"]))
df = pii_df
