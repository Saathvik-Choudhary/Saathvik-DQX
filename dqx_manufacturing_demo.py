...
# MAGIC %md
# MAGIC ## DAIS 2025 DQX Test Session 
# MAGIC This notebook validates DQX features for a fictional Manufacturing Company "Machina Metrics". <br>
...
# MAGIC %md
# MAGIC ### Environment <br>
# MAGIC DQX is installed at the cluster level. This notebook validates rules defined in YAML.
...
# Overwrite with open-source predictive maintenance data for the test
pm_url = "https://raw.githubusercontent.com/rockyfjord/predictive-maintenance/master/maintenance.csv"
pm_df = (
...
sensor_df = pm_df.select(
    concat(lit("SEN-"), col("UDI").cast("string")).alias("sensor_id"),
    col("Product ID").alias("machine_id"),
    lit("process_temperature").alias("sensor_type"),
    col("Process temperature [K]").cast("double").alias("reading_value"),
    current_timestamp().alias("reading_timestamp"),
    date_sub(current_date(), 30).alias("calibration_date"),
    col("Torque [Nm]").cast("int").alias("battery_level"),
    when(col("Type") == "L", "Zone-A").when(col("Type") == "M", "Zone-B").otherwise("Zone-C").alias("facility_zone"),
    (col("Machine failure") == 0).alias("is_active"),
    concat(lit("v"), col("Type"), lit(".1")).alias("firmware_version"),
    current_date().alias("ingest_date"),
)

invalid_sensor_rows = [
    ("SEN-NULL", None, "process_temperature", -5.0, current_timestamp(), None, -1, "Zone-X", True, "1.0.0", current_date()),
]
invalid_sensor_df = spark.createDataFrame(invalid_sensor_rows, sensor_df.schema)
sensor_df = sensor_df.unionByName(invalid_sensor_df)
sensor_df.write.mode("overwrite").saveAsTable(sensor_table)
...
# Overwrite with open-source predictive maintenance data for the test
maintenance_df = pm_df.select(
    concat(lit("MTN-"), col("UDI").cast("string")).alias("maintenance_id"),
    col("Product ID").alias("machine_id"),
    when(col("Machine failure") == 1, "corrective").otherwise("preventive").alias("maintenance_type"),
    date_sub(current_date(), (col("Tool wear [min]").cast("int") % 30)).alias("maintenance_date"),
    col("Tool wear [min]").cast("int").alias("duration_minutes"),
    (col("Torque [Nm]") * 10).cast("decimal(10,2)").alias("cost"),
    date_add(current_date(), 90).alias("next_scheduled_date"),
    concat(lit("WO-"), col("UDI").cast("string")).alias("work_order_id"),
    (col("Machine failure") == 0).alias("safety_check_passed"),
    array(lit("filter"), lit("gasket")).alias("parts_list"),
    current_date().alias("ingest_date"),
)

invalid_maintenance_rows = [
    ("MTN-NULL", None, None, datetime.today().date(), -30, Decimal("-500.00"), datetime.today().date(), "INVALID", None, [], datetime.today().date()),
]
invalid_maintenance_df = spark.createDataFrame(invalid_maintenance_rows, maintenance_df.schema)
maintenance_df = maintenance_df.unionByName(invalid_maintenance_df)
maintenance_df.write.mode("overwrite").saveAsTable(maintenance_table)
