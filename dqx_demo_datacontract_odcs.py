...
# MAGIC %md
# MAGIC # DQX Data Contract Integration Test (ODCS v3.x)
# MAGIC
...
# MAGIC ## Environment
# MAGIC
# MAGIC DQX is installed at the cluster level with data contract and LLM extras.
...
# MAGIC This example uses a simple iris measurements contract.
...
kind: DataContract
apiVersion: v3.0.2
id: urn:datacontract:botany:iris
name: Iris Measurements
version: 1.0.0
status: active
domain: botany
dataProduct: iris_data_product
tenant: Data Engineering Team

description:
  purpose: Iris flower measurements for demo analytics
...
schema:
  - name: iris
    physicalName: iris_table
    physicalType: table
    description: Iris measurements dataset
    ...
    quality:
      ...
      - type: custom
        engine: dqx
        description: Ensure iris dataset contains data
        implementation:
          criticality: error
          name: iris_dataset_not_empty
          check:
      ...
      - type: text
        description: "Petal length should be greater than or equal to petal width"
"""
...
# Add a few invalid records for demonstration
invalid_rows = [
    (3.0, 2.0, 1.0, 0.2, "setosa"),    # sepal_length too low
    (5.5, 3.0, 8.0, 0.2, "virginica"),  # petal_length too high
    (5.1, 3.5, 1.4, 0.2, "unknown"),    # invalid species
    (None, 3.0, 1.4, 0.2, "setosa"),    # missing sepal_length
]
invalid_df = spark.createDataFrame(invalid_rows, schema=iris_df.schema)

df = iris_df.unionByName(invalid_df)
display(df)
